...
async function officialFlight(pilotId: string, flightDispatchId: string | null, ofpBriefingId: string | null) {
  ...
  const dispatch = await prisma.flightDispatch.findFirst({ where: { id: flightDispatchId, pilotId, dataOrigin: "HISPAFLY_NATIVE", isCurrent: true, status: "RELEASED", completedAt: null, cancelledAt: null, ofpBriefing: { ...(ofpBriefingId ? { id: ofpBriefingId } : {}) } }, include: { flightOffer: true, flight: true, aircraft: true, ofpBriefing: true } });
  if (!dispatch?.ofpBriefing || !dispatch.flight || !dispatch.aircraft) throw new EfbApiError(403, "OFFICIAL_FLIGHT_NOT_AVAILABLE", "A current Native Released Dispatch is required.");
  return dispatch;
}

export async function getActivePerformanceFlight(pilotId: string) {
  const dispatch = await prisma.flightDispatch.findFirst({ where: { pilotId, dataOrigin: "HISPAFLY_NATIVE", isCurrent: true, status: "RELEASED", completedAt: null, cancelledAt: null }, include: { flight: true, aircraft: true, ofpBriefing: true }, orderBy: { dispatchedAt: "desc" } });
  if (!dispatch?.ofpBriefing || !dispatch.flight || !dispatch.aircraft) return { active: false, mode: "MANUAL" };
  const identity = normalizeFlightIdentity({ flightNumber: dispatch.flight.flightNumber, callsign: dispatch.flight.callsign });
  ...
  return { contractVersion: "1.0", active: true, mode: "OFFICIAL", phase: "POST_DISPATCH", flightDispatchId: dispatch.id, bookingId: dispatch.bookingId, flightId: dispatch.flight.id, ofpBriefingId: dispatch.ofpBriefing.id, flightNumber: identity.commercialFlightNumber, callsign: identity.atcCallsign, departureAirportId: dispatch.flight.departureAirportId, departureIcao: dispatch.flight.departureIcao, arrivalAirportId: dispatch.flight.arrivalAirportId, arrivalIcao: dispatch.flight.arrivalIcao, aircraftId: dispatch.aircraft.id, aircraftType: dispatch.aircraft.aircraftType, aircraftRegistration: dispatch.aircraft.registration, takeoffWeightKg: numericValue(summary.tow), landingWeightKg: numericValue(summary.landingWeight), plannedDepartureRunway: null, plannedArrivalRunway: null, ofpStatus: dispatch.ofpBriefing.status, dispatchStatus: dispatch.status, readyForDepartureStatus: readiness?.status ?? "PENDING" };
}
...
export async function runPerformanceCalculation(pilotId: string, type: PerformanceType, body: JsonRow) {
  ...
  if (dispatch) {
    const expectedAirport = type === "TAKEOFF" ? dispatch.flight!.departureIcao : dispatch.flight!.arrivalIcao;
    ...
    if (dispatch.aircraft!.aircraftType && aircraftType !== dispatch.aircraft!.aircraftType.toUpperCase()) throw new EfbApiError(400, "OFFICIAL_FLIGHT_MISMATCH", `Official aircraft must be ${dispatch.aircraft!.aircraftType}.`);
    if (dispatch.aircraft!.registration && aircraftRegistration && aircraftRegistration.toUpperCase() !== dispatch.aircraft!.registration.toUpperCase()) throw new EfbApiError(400, "OFFICIAL_FLIGHT_MISMATCH", `Official registration must be ${dispatch.aircraft!.registration}.`);
  }
  ...
  try {
    ...
    const calculation = await prisma.efbPerformanceCalculation.create({ data: { pilotId, ofpBriefingId: dispatch?.ofpBriefing?.id, flightDispatchId: dispatch?.id, type, mode, airportIcao, runway, aircraftType, aircraftRegistration, weightKg: weightKg === null ? null : Math.round(weightKg), input: body as unknown as Prisma.InputJsonValue, result: raw as unknown as Prisma.InputJsonValue, status: normalized.status, warningLevel: normalized.warnings.length ? "WARNING" : null, errorMessage: normalized.message, officialForDeparture: type === "TAKEOFF" && mode === "OFFICIAL" } });
