...
import { assertDraftEditable, assertScheduleDeletable, normalizeScheduleDraftInput, scheduleCreatePolicy, ScheduleManagementError } from "./management-rules.ts";
...
assert.doesNotThrow(() => assertScheduleDeletable("DRAFT", 0));
assert.doesNotThrow(() => assertScheduleDeletable("ARCHIVED", 0));
for (const status of ["ACTIVE", "SUSPENDED", "EXPIRED"]) assert.throws(() => assertScheduleDeletable(status, 0), /Solo se pueden eliminar/);
assert.throws(() => assertScheduleDeletable("DRAFT", 1), /vuelos relacionados/);
...
assert.match(management, /SCHEDULE_DELETED/);
assert.match(management, /status: "ARCHIVED", archivedAt: new Date\(\)/);
assert.match(management, /flights: \{ none: \{\} \}/, "hard deletion rechecks that no related flights exist");
