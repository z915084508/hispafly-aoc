...
for(const text of ["planner\"","list\"","unassigned\"","publication\"","PLANIFICADOR SEMANAL","LISTA","SIN AERONAVE","PENDIENTES DE PUBLICACIÓN","NUEVA PROGRAMACIÓN"])assert.match(workspace,new RegExp(text));
assert.match(workspace,/view = \["planner"/);assert.match(workspace,/WeeklyPlanner/);assert.match(workspace,/ScheduleForm/);assert.match(workspace,/PublicationQueue/);assert.match(workspace,/programacion-workspace-split/);assert.match(workspace,/panel-open/);
...
assert.match(form,/hiddenFields/);assert.match(form,/rotationContext/);assert.match(form,/Comprobar programación/);assert.match(actions,/workspace\(form/);assert.match(actions,/createFlightScheduleDraft/);assert.match(actions,/updateFlightScheduleDraft/);assert.match(actions,/bulkPublishProgramacionAction/);
assert.match(publicationQueue,/PUBLICAR SELECCIONADAS/);assert.match(publicationQueue,/SELECCIONAR TODO/);
...
console.log("Unified Programación workspace contracts passed (67 focused assertions).");
