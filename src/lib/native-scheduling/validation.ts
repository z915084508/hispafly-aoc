...
export function validateProposedScheduleWithContext(proposed: ProposedFlightSchedule, context: ScheduleValidationContext, options: { includeExistingGeneratedFlights?: boolean } = {}): ScheduleValidationResult {
  ...
  if (!Number.isInteger(proposed.bookingOpenOffsetMinutes) || proposed.bookingOpenOffsetMinutes < 0) errors.push(issue("INVALID_BOOKING_OPEN_OFFSET", "Booking open offset must be a non-negative integer.", proposed));
  if (!Number.isInteger(proposed.bookingCloseOffsetMinutes) || proposed.bookingCloseOffsetMinutes < 0) errors.push(issue("INVALID_BOOKING_CLOSE_OFFSET", "Booking close offset must be a non-negative integer.", proposed));
  if (Number.isInteger(proposed.bookingOpenOffsetMinutes) && Number.isInteger(proposed.bookingCloseOffsetMinutes) && proposed.bookingOpenOffsetMinutes <= proposed.bookingCloseOffsetMinutes) errors.push(issue("INVALID_BOOKING_WINDOW", "Booking opening must occur before booking closing.", proposed));
  if (!Number.isInteger(proposed.generationHorizonDays) || proposed.generationHorizonDays < 1 || proposed.generationHorizonDays > 365) errors.push(issue("INVALID_GENERATION_HORIZON", "Generation horizon must be between 1 and 365 days.", proposed));
