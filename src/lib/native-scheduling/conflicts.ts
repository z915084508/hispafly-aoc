...
export function validateAircraftScheduleConflicts(proposed: ProposedFlightSchedule, route: { departureAirportId: string | null; arrivalAirportId: string | null }, schedules: ExistingSchedule[]): ScheduleValidationIssue[] {
  ...
  const relevant = schedules.filter((schedule) => schedule.assignedAircraftId === proposed.assignedAircraftId && schedule.id !== proposed.scheduleId && schedule.status !== "ARCHIVED" && effectivePeriodsOverlap(proposed.effectiveFrom, proposed.effectiveUntil, schedule.effectiveFrom, schedule.effectiveUntil));
  ...
  return [...new Map(issues.map((issue) => [key(issue), issue])).values()];
}
