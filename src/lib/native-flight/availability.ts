...
import { ASSIGNABLE_AIRCRAFT_STATUSES } from "./fleet-aircraft-rules";
...
export async function checkAircraftAvailability(input: { aircraftId: string; routeId?: string | null; departureAirportId?: string | null; startsAt: Date; endsAt: Date; ferryFlight?: boolean }): Promise<AircraftAvailabilityResult> {
  ...
  const aircraft = await prisma.aircraft.findUnique({ where: { id: input.aircraftId }, include: { nativeFleet: true, currentAirport: true, conditionSnapshot: true } });
  if (!aircraft) return { allowed: false, blockingReasons: ["Aircraft does not exist."], warnings, checkedAt };
  if (!ASSIGNABLE_AIRCRAFT_STATUSES.has(aircraft.operationalStatus) && !(input.ferryFlight && aircraft.operationalStatus === "FERRY_ONLY")) blockingReasons.push(`Aircraft status ${aircraft.operationalStatus} is not assignable.`);
  ...
  if (input.departureAirportId && aircraft.currentAirportId !== input.departureAirportId) blockingReasons.push("Aircraft is not at the required departure airport.");
  if (!aircraft.currentAirportId) warnings.push("Aircraft current airport is unknown.");
