    ...
    try {
      const created = await prisma.flight.create({
        data: {
          ...
          scheduledDurationMinutes: schedule.scheduledDurationMinutes,
          flightNumber: schedule.flightNumber || route.flightNumber || schedule.code,
          callsign: schedule.callsign || route.callsign || schedule.flightNumber || route.flightNumber || schedule.code,
