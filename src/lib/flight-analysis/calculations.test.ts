...
assert.equal(efficiencyScore({ fuelDiffPercent: -66.5, blockTimeDiffMinutes: -15, landingRate: -363 }), 100);
assert.ok(efficiencyScore({ fuelDiffPercent: 10, blockTimeDiffMinutes: 15, landingRate: -500 }) < 100);
console.log("Flight analysis calculations: 9 assertions passed.");

assert.equal(efficiencyScore({fuelDiffPercent:0,blockTimeDiffMinutes:0,landingRate:-950}),100);
