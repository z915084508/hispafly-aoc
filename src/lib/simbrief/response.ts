...
export function summarizeSimbriefOfp(snapshot: unknown) {
  ...
  return {
    ...
    cruiseAltitude: valueAt(root, ["general", "initial_altitude"], ["general", "cruise_altitude"], ["params", "fl"]),
    costIndex: valueAt(root, ["general", "costindex"], ["general", "cost_index"], ["params", "civalue"]),
    ...
    departureMetar: valueAt(root, ["origin", "metar"], ["weather", "orig_metar"], ["weather", "origin_metar"]),
    arrivalMetar: valueAt(root, ["destination", "metar"], ["weather", "dest_metar"], ["weather", "destination_metar"]),
    alternateMetar: valueAt(root, ["alternate", "metar"], ["weather", "altn_metar"], ["weather", "alternate_metar"])
      ?? valueAt(firstAlternate, ["metar"], ["weather", "metar"]),
    passengers: valueAt(root, ["weights", "pax_count"], ["weights", "passengers"], ["general", "passengers"], ["params", "pax"], ["pax_count"]),
    baggage: valueAt(root, ["weights", "bag_weight"], ["weights", "baggage"], ["weights", "baggage_weight"]),
    cargo: valueAt(root, ["weights", "cargo"], ["weights", "freight"]),
    payload: valueAt(root, ["weights", "payload"], ["weights", "est_payload"], ["general", "payload"]),
    dow: valueAt(root, ["weights", "oew"], ["weights", "dow"], ["weights", "operating_empty_weight"]),
    ...
    taxiFuel: valueAt(root, ["fuel", "taxi"], ["fuel", "taxi_out"]),
    tripFuel: valueAt(root, ["fuel", "enroute_burn"], ["fuel", "trip"], ["fuel", "trip_fuel"]),
    contingencyFuel: valueAt(root, ["fuel", "contingency"], ["fuel", "cont"]),
    alternateFuel: valueAt(root, ["fuel", "alternate_burn"], ["fuel", "alternate"]),
