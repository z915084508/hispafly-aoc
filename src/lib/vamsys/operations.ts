...
export async function syncOperationsPilotsIncremental(options: { maxPages?: number; cron?: boolean } = {}) {
  ...
  try {
    ...
    await writeAuditLogSafely({ action: "VAMSYS_OPERATIONS_PILOT_CRON_FAILED", entityType: "Pilot", message: `Fall贸 el cron de pilotos Operations: ${message}`, metadata: { imported, updated, skipped, errors: errors.length } });
  }
