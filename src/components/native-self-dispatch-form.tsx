...
type AmnAllocation = { payloadRequestId: string; marketSnapshotId: string; loadStage: string; passengers: number; cargoWeightKg: number; cargoVolumeM3: number; sellableSeats: number; maximumCargoWeightKg: number; maximumTrafficPayloadKg: number; estimatedTrafficPayloadKg: number };
...
export function NativeSelfDispatchForm({ routes, aircraft, idempotencyKey, simbriefConnected, pilotAirportIcao }: { routes: RouteOption[]; aircraft: AircraftOption[]; idempotencyKey: string; simbriefConnected: boolean; pilotAirportIcao: string }) {
  ...
  function clearAmnAllocation() { setAmnAllocation(null); setAmnToken(""); setAmnError(""); }

  async function generateAmnPayload() {
    if (!routeId || !aircraftId || !departureAt) return;
  ...
  return <form className="pilot-booking-form native-self-dispatch" action={createNativeSelfDispatchAction}>
    ...
    <fieldset><legend><span>01</span> Route and aircraft</legend><div className="pilot-booking-grid">
      ...
      <label>Available aircraft<select name="aircraftId" value={aircraftId} onChange={(event) => { setAircraftId(event.target.value); clearAmnAllocation(); }} disabled={!route} required><option value="">{route ? compatibleAircraft.length ? "Select aircraft" : `No compatible aircraft at ${departure}` : "Match route first"}</option>{compatibleAircraft.map((item) => <option value={item.id} key={item.id}>{item.registration ?? item.id} · {item.aircraftType ?? "Type pending"} · {item.seatCapacity} seats{item.stale ? " · stale position" : ""}{item.external ? " · external source" : ""}</option>)}</select></label>
    </div></fieldset>
    ...
    <fieldset><legend><span>02</span> Schedule</legend><div className="pilot-booking-grid">
      <label className="span-2">Departure date and time (UTC)<input type="datetime-local" name="departureAt" value={departureAt} onChange={(event) => { setDepartureAt(event.target.value); clearAmnAllocation(); }} required/></label>
    ...
    <fieldset><legend><span>03</span> AMN Payload and flight plan</legend>
      ...
      <div className="button-row"><button className="button secondary" type="button" onClick={generateAmnPayload} disabled={!routeId || !aircraftId || !departureAt || amnLoading}>{amnLoading ? "Requesting AMN traffic…" : amnAllocation ? "Refresh AMN Payload" : "Auto Generate Payload"}</button><span className="meta">AMN determines passenger and commercial cargo demand from the selected flight, date and aircraft capacity.</span></div>
      ...
      <label>Baggage per passenger kg<input value={amnAllocation ? baggagePerPassenger : ""} readOnly/></label>
