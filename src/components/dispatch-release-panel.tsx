...
export function DispatchReleasePanel({ release }: { release: ReleaseRecord | null }) {
  ...
  const passed = items.filter((item) => item.status === "OK" || item.status === "NOT_REQUIRED").length;
  return <section className="card">
    ...
    <div className="workflow-summary"><div><span>Automatic checks</span><strong>{passed}/{items.length} ready</strong></div><div><span>Warnings</span><strong>{warnings.length}</strong></div><div><span>Blocking items</span><strong>{blocking.length}</strong></div></div>
    ...
    <details className="release-check-details"><summary>View complete technical checklist</summary><div className="table-wrapper"><table><thead><tr><th>Checklist item</th><th>Status</th><th>Details</th></tr></thead><tbody>{items.map((item) => <tr key={item.key}><td>{item.label}</td><td><Badge tone={tone(item.status)}>{item.status}</Badge></td><td>{item.detail}</td></tr>)}</tbody></table></div></details>
  </section>;
}
