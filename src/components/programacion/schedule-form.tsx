...
export function ScheduleForm({
  ...
  return <form action={action} className="programacion-form">
    ...
    <fieldset><legend>Identidad y ruta</legend><div className="form-grid"><label>Código interno<input name="code" value={code} placeholder="Se genera al seleccionar la ruta" onChange={(event) => { setCode(event.target.value); setCodeTouched(true); }}/>{!value.id && <span className="secondary">{codeTouched ? "Código personalizado. Debe ser único." : "Automático: usa el número de vuelo y añade -02, -03… si ya existe."}</span>}</label><label>Descripción interna<input name="name" defaultValue={value.name ?? ""}/></label><label className="programacion-wide">Ruta<select name="routeId" required value={routeId} onChange={(event) => { const selected = routes.find((item) => item.id === event.target.value); setRouteId(event.target.value); setFleetIds((current) => selected?.compatibleFleetIds.length ? current.filter((id) => selected.compatibleFleetIds.includes(id)) : current); if (selected?.duration) setDuration(selected.duration); }}><option value="">Selecciona una ruta</option>{routes.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label></div>{route && <div className="programacion-route-summary"><span>Vuelo <strong>{route.flightNumber ?? "—"}</strong></span><span>Callsign <strong>{route.callsign ?? "—"}</strong></span><span>Trayecto <strong>{route.departure} → {route.arrival}</strong></span><span>Duración base <strong>{route.duration ? hhmm(route.duration) : "—"}</strong></span></div>}</fieldset>
