...
export default async function PortalSelection({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="portal-gateway">
    ...
    {denied && <div className="feedback error">ADMIN PORTAL requires the Admin account.</div>}
    <section className="portal-choices">
      ...
      <Link className="portal-choice staff-choice" href="/admin-login"><span className="portal-number">02</span><div><small>ADMINISTRATION</small><h2>ADMIN PORTAL</h2><p>Operations, pilots, payroll, reports and system settings.</p></div><strong>LOGIN →</strong></Link>
    </section>
