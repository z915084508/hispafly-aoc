...
import { importLegacyRouteCsvAction } from "./actions";
...
export default async function RoutesPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <>
    ...
    {staffHasPermission(staff, "ROUTE_SYNC") && <details className="card route-import-card"><summary><span><strong>Import historical vAMSYS routes</strong><small>Offline CSV migration · no vAMSYS connection</small></span><b>＋</b></summary><form action={importLegacyRouteCsvAction} className="route-import-form"><div><label>vAMSYS Route CSV<input name="routeCsv" type="file" accept=".csv,text/csv" required/></label><p>Preview validates Airport and Fleet references and reports duplicates. Import is idempotent by vAMSYS Route ID and writes an audit event.</p></div><div className="button-row"><button className="button secondary" name="mode" value="preview">Preview CSV</button><button className="button" name="mode" value="import">Import routes</button></div></form></details>}
    ...
    <div className="table-wrap"><table><thead><tr><th>Route</th><th>Airports</th><th>Compatible fleets</th><th>Duration</th><th>Status</th><th>Source</th><th>Effective period</th></tr></thead><tbody>{result.rows.map((route) => <tr key={route.id}><td><Link href={`/staff/routes/${route.id}`}><strong>{route.routeCode ?? "—"}</strong></Link></td><td>{route.departureAirport?.icao ?? route.departure} → {route.arrivalAirport?.icao ?? route.arrival}</td><td>{route.fleetAssignments.map(({ fleet }) => fleet.code ?? fleet.name).filter(Boolean).join(", ") || "—"}</td><td>{route.scheduledDurationMinutes ? `${route.scheduledDurationMinutes} min` : "—"}</td><td><span className="badge">{route.operationalStatus}</span></td><td><span className="badge">{route.dataOrigin}</span>{route.vamsysRouteId && <><br/><small>Legacy</small></>}</td><td>{route.effectiveFrom?.toLocaleDateString() ?? "Open"} → {route.effectiveUntil?.toLocaleDateString() ?? "Open"}</td></tr>)}</tbody></table></div>
