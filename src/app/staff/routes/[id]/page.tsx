...
export default async function RouteDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  return <>
    ...
    <div className="detail-grid">
      <section className="card"><h2>Route record</h2><p>Route code: {route.routeCode ?? "—"}</p>{route.dataOrigin !== "HISPAFLY_NATIVE" && <><p>Legacy flight number: {route.flightNumber ?? "—"}</p><p>Legacy callsign: {route.callsign ?? "—"}</p></>}<p>Legacy reference: {route.vamsysRouteId ?? "—"}</p><p>Source: {route.dataOrigin}</p><p>Operational flight numbers and callsigns belong to Programación.</p></section>
