...
const input=(f:FormData)=>{const requestedMode=v(f,"operationMode").toUpperCase();const deliveryActive=requestedMode==="DELIVERY";const postDeliveryMode=(v(f,"postDeliveryOperationMode").toUpperCase()||"SCHEDULED") as AircraftOperationMode;const operationMode=(deliveryActive?postDeliveryMode:requestedMode) as AircraftOperationMode;return{registration:v(f,"registration"),aircraftType:v(f,"aircraftType"),fleetId:v(f,"fleetId"),hubAirportIds:f.getAll("hubAirportIds").map(String),operationMode,name:v(f,"name"),serialNumber:v(f,"serialNumber"),selcal:v(f,"selcal"),deliveryDate:d(f,"deliveryDate"),inServiceDate:d(f,"inServiceDate"),cabinConfiguration:v(f,"cabinConfiguration"),seatCapacity:n(f,"seatCapacity"),cargoCapacityKg:n(f,"cargoCapacityKg"),internalNotes:v(f,"internalNotes"),delivery:{active:deliveryActive,originIcao:v(f,"deliveryOriginIcao"),destinationIcao:v(f,"deliveryDestinationIcao"),postDeliveryOperationMode:postDeliveryMode}}};
