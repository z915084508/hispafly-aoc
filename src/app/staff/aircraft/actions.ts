...
import { changeAircraftStatus, copyAircraftToNative, correctAircraftTotals, createNativeAircraft, setAircraftHubs, setNativeAircraftLocation, updateNativeAircraft } from "@/lib/native-flight/aircraft";
...
const input=(f:FormData)=>({registration:v(f,"registration"),aircraftType:v(f,"aircraftType"),fleetId:v(f,"fleetId"),hubAirportIds:f.getAll("hubAirportIds").map(String),operationMode:v(f,"operationMode") as AircraftOperationMode,name:v(f,"name"),serialNumber:v(f,"serialNumber"),selcal:v(f,"selcal"),deliveryDate:d(f,"deliveryDate"),inServiceDate:d(f,"inServiceDate"),cabinConfiguration:v(f,"cabinConfiguration"),seatCapacity:n(f,"seatCapacity"),cargoCapacityKg:n(f,"cargoCapacityKg"),internalNotes:v(f,"internalNotes")});
...
export async function copyAircraftAction(f:FormData){const id=v(f,"id");let t=`/staff/aircraft/${id}`;try{const s=await requireStaffPermission("AIRCRAFT_CREATE",{entityType:"Aircraft",entityId:id,attemptedAction:"copy Legacy Aircraft"}),x=await copyAircraftToNative(id,v(f,"registration"),v(f,"fleetId"),s);t=`/staff/aircraft/${x.id}?success=Native%20Aircraft%20created.`}catch(e){t+=`?error=${encodeURIComponent(msg(e,"Copy failed."))}`}redirect(t)}
