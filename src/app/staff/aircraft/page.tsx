...
import { aircraftIsInDelivery } from "@/lib/native-flight/aircraft-delivery";
...
export default async function Page({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  return <>
    ...
    <div className="table-wrap"><table><thead><tr><th>Registration</th><th>Fleet/type</th><th>HUBS</th><th>Location</th><th>Status</th><th>Maintenance</th><th>Hours/cycles</th><th>Last report</th></tr></thead><tbody>{result.rows.map((aircraft)=>{const native=aircraft.dataOrigin!=="VAMSYS_LEGACY",deliveryActive=aircraftIsInDelivery(aircraft.rawData);return <tr key={aircraft.id}><td><Link href={`/staff/aircraft/${aircraft.id}`}><strong>{aircraft.registration??"—"}</strong></Link>{deliveryActive&&<div className="meta"><strong>DELIVERY</strong></div>}</td><td>{aircraft.nativeFleet?.code??aircraft.fleetName??"—"} / {aircraft.aircraftType??"—"}</td><td><InlineHubs aircraftId={aircraft.id} returnTo={returnTo} canManage={native&&canEditHubs} airports={airports} hubs={aircraft.hubs}/></td><td><InlineLocation aircraftId={aircraft.id} returnTo={returnTo} canManage={native&&canManageLocation} airports={airports} airportId={aircraft.currentAirportId} label={aircraft.currentAirport?.icao??aircraft.locationSnapshot?.currentAirportIcao??"Unknown"} status={aircraft.operationalStatus}/></td><td>{deliveryActive&&<div className="meta">DELIVERY · ferry only</div>}<InlineStatus aircraftId={aircraft.id} returnTo={returnTo} canManage={native&&canManageStatus} status={aircraft.operationalStatus}/></td><td>{aircraft.conditionSnapshot?.maintenanceStatus??"NONE"}</td><td>{(aircraft.totalFlightMinutes/60).toFixed(1)} / {aircraft.totalCycles}</td><td>{aircraft.locationSnapshot?.lastReportAt?.toLocaleString()??"—"}</td></tr>})}</tbody></table></div>
