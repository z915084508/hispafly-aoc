...
import { AirportMapView } from "@/components/airport-map-view";
...
export default async function AirportsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [result, mappedAirports] = await Promise.all([
    listAirports({ search: query.search, country: query.country, status: query.status as AirportStatus | undefined, dataOrigin: query.source, page: Number(query.page) || 1 }),
    prisma.airport.findMany({ where: { latitude: { not: null }, longitude: { not: null } }, select: { id: true, icao: true, iata: true, name: true, city: true, country: true, latitude: true, longitude: true, status: true }, orderBy: { icao: "asc" } }),
  ]);
  return <>
    ...
    <AirportMapView airports={mappedAirports.map((airport) => ({ ...airport, latitude: airport.latitude!, longitude: airport.longitude! }))}/>
