...
export default async function AirportDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  if (!airport) notFound();
  const editable = airport.status !== "ARCHIVED" && staffHasPermission(staff, "AIRPORT_EDIT");
  return <>
    ...
    {airport.dataOrigin === "VAMSYS_LEGACY" && <div className="notice">Imported from the former vAMSYS system. This is provenance metadata only; the airport is available to HispaFly operations.</div>}
