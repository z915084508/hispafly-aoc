...
export default async function Page({searchParams}:{searchParams:Promise<{fleet?:string;saved?:string;error?:string}>}){
  ...
  return <>
    ...
    <PageHeading eyebrow="FLIGHT OPERATIONS" title="PIREP Scoring Rules" copy="FOQA v2: episode severity bands, SOP cap −8, bonus cap +5. Rule enablement may vary by fleet."/>
    ...
    <form action={saveScoringPolicy} className="route-form">
      ...
      <section className="card"><div className="route-form-grid"><label>Starting score<input name="startingScore" type="number" min="0" max="100" value={100} readOnly/></label><label>Operational weight %<input name="operationalWeight" type="number" min="0" max="100" value={70} readOnly/></label><label>Efficiency weight %<input name="efficiencyWeight" type="number" min="0" max="100" value={30} readOnly/></label><div><strong>{ownPolicy?`Version ${ownPolicy.version}`:fleetId?"Inherited from global":effectivePolicy?`Version ${effectivePolicy.version}`:"Built-in defaults"}</strong><p className="meta">{selected?`Fleet: ${selected.code||selected.name}`:"All fleets"}</p></div></div></section>
      <div className="scoring-groups">{groups.map(group=><section className="card scoring-group" key={group}><h2>{group}</h2><div className="table-wrapper"><table><thead><tr><th>Use</th><th>Condition</th><th>Action</th><th>Points</th></tr></thead><tbody>{rules.filter(rule=>rule.group===group).map(rule=><tr key={rule.code}><td><input type="checkbox" name={`enabled:${rule.code}`} defaultChecked={rule.enabled} disabled={rule.availability==="PLANNED"}/></td><td className="scoring-rule-condition"><strong>{rule.label}</strong><div className="meta">{rule.condition}</div><span className={`scoring-status ${rule.availability.toLowerCase()}`}>{rule.availability==="ACTIVE"?"ACARS DATA READY":"DATA SOURCE PLANNED"}</span></td><td><select name={`action:${rule.code}`} defaultValue={rule.action} disabled={!["SLEW","TELEPORT","MID_AIR_REFUELING"].includes(rule.code)}><option>ADD</option><option>DEDUCT</option><option>REVIEW</option><option>INVALIDATE</option><option>NONE</option></select></td><td><span>{rule.points || "Severity band"}</span></td></tr>)}</tbody></table></div></section>)}</div>
