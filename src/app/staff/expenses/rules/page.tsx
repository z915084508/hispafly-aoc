...
import { saveAircraftProfileAction, saveAirportCategoryChargeProfileAction, saveAirportChargeProfileAction, saveAirspaceChargeProfileAction } from "./actions";
...
async function getRules() {
  if (!process.env.DATABASE_URL) return { categories: [], airports: [], airspaces: [], aircraft: [] };
  const [categories, airports, airspaces, aircraft] = await Promise.all([
    prisma.airportCategoryChargeProfile.findMany({ orderBy: { airportCategory: "asc" } }).catch(() => []),
  ...
  return { categories, airports, airspaces, aircraft };
}

type Rules = Awaited<ReturnType<typeof getRules>>;

type ChargeRuleValues = {
  landingRatePerTonneCents: number;
  passengerFeeCents: number;
  passengerServiceFeeCents: number;
  parkingRatePerHourCents: number;
  terminalAtcUnitRateCents: number;
};

function ChargeFields({ rule }: { rule?: ChargeRuleValues }) {
  return <>
...
function AirportCategoryRuleForm({ rule }: { rule?: Rules["categories"][number] }) {
  return <form action={saveAirportCategoryChargeProfileAction} className="expense-rule-form category-rule-form">
    <label>Categoría<input name="airportCategory" defaultValue={rule?.airportCategory ?? ""} placeholder="major" required /></label>
    <ChargeFields rule={rule} />
    <SubmitButton className="action-button approve" pendingChildren="Guardando...">Guardar</SubmitButton>
  </form>;
}

function AirportRuleForm({ rule }: { rule?: Rules["airports"][number] }) {
  return <form action={saveAirportChargeProfileAction} className="expense-rule-form airport-rule-form">
...
function AirspaceRuleForm({ rule }: { rule?: Rules["airspaces"][number] }) {
...
function AircraftRuleForm({ rule }: { rule?: Rules["aircraft"][number] }) {
...
export default async function StaffExpenseRulesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return <>
    <style>{`
      ...
      .expense-rule-form.category-rule-form, .expense-rule-form.airport-rule-form { grid-template-columns: repeat(3, minmax(120px, 1fr)); }
      ...
      @media (max-width: 1180px) { .expense-rules-grid { grid-template-columns: 1fr; } .expense-rule-form, .expense-rule-form.category-rule-form, .expense-rule-form.airport-rule-form, .expense-rule-form.compact, .expense-rule-form.aircraft-rule-form { grid-template-columns: 1fr 1fr; } }
      @media (max-width: 720px) { .expense-rule-form, .expense-rule-form.category-rule-form, .expense-rule-form.airport-rule-form, .expense-rule-form.compact, .expense-rule-form.aircraft-rule-form { grid-template-columns: 1fr; } .expense-rules-header { align-items: flex-start; flex-direction: column; } }
    `}</style>
    <div className="expense-rules-header">
      <PageHeading eyebrow="ECONOMY RULES" title="Reglas de gastos" copy="Mantén reglas por categoría de aeropuerto, overrides especiales, ATC unit rates y perfiles fallback para calcular CompanyExpense desde PIREPs aceptados." />
    ...
    <section className="card rule-section">
      <div className="card-header"><h2 className="card-title">Nueva / actualizar categoría de aeropuerto</h2><span className="meta">AirportCategoryChargeProfile</span></div>
      <p className="rule-help">Regla principal recomendada. Úsala para categorías como mega_hub, major, medium, small, regional o standard. Los importes se introducen en euros y se guardan como céntimos EUR.</p>
      <AirportCategoryRuleForm />
    </section>

    <section className="card rule-section">
      <div className="card-header"><h2 className="card-title">Airport category charge profiles</h2><span className="meta">{rules.categories.length} reglas</span></div>
      {rules.categories.length === 0 ? <div className="empty-state">Todavía no hay reglas por categoría.</div> : <DataTable headers={["Categoría", "Landing", "Pax", "Service", "Parking", "Terminal ATC", "Editar"]} rows={rules.categories.map((rule) => [
        ...
        <AirportCategoryRuleForm key="edit" rule={rule} />,
      ])} />}
    </section>

    <section className="card rule-section">
      <div className="card-header"><h2 className="card-title">Override especial por aeropuerto</h2><span className="meta">AirportChargeProfile</span></div>
      <p className="rule-help">Solo para aeropuertos que necesiten una tarifa diferente a su categoría. Si no existe override, AOC usará automáticamente la categoría calculada por ICAO/volumen.</p>
      <AirportRuleForm />
    </section>
    ...
    <section className="card rule-section">
      <div className="card-header"><h2 className="card-title">Airport override profiles</h2><span className="meta">{rules.airports.length} reglas especiales</span></div>
      {rules.airports.length === 0 ? <div className="empty-state">Todavía no hay overrides por aeropuerto.</div> : <DataTable headers={["ICAO", "Categoría", "Landing", "Pax", "Service", "Parking", "Terminal ATC", "Editar"]} rows={rules.airports.map((rule) => [
