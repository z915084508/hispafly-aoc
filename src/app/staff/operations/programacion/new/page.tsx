...
export default async function NewProgramacionPage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  const value = { code: query.code, routeId: reverseRoutes.length === 1 ? reverseRoutes[0].id : undefined, assignedAircraftId: query.aircraftId, defaultFleetId: query.defaultFleetId, daysOfWeek: days, departureTimeMinutesUtc: query.departureTimeMinutesUtc ? Number(query.departureTimeMinutesUtc) : undefined, effectiveFrom: query.effectiveFrom ?? today, effectiveUntil: query.effectiveUntil };
  return <>
    ...
    <ScheduleForm action={createProgramacionAction} value={value} {...options} submitLabel="Guardar borrador" allowReturnCreation={!query.reverseOf}/>
  </>;
}
