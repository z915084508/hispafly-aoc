...
export async function publishProgramacionAction(form: FormData) { const id = value(form, "id"); let target = `/staff/operations/programacion/${id}`; try { const staff = await requireStaffPermission("SCHEDULE_STATUS_MANAGE", { entityType: "FlightSchedule", entityId: id, attemptedAction: "publish Programación" }); const result = await publishFlightSchedule({ scheduleId: id, warningFingerprint: value(form, "acknowledged") === "yes" ? value(form, "warningFingerprint") : undefined, actor: staff }); target += `?published=1&created=${result.generation.created}&existing=${result.generation.existing}`; revalidatePath("/staff/operations/programacion"); revalidatePath(`/staff/operations/programacion/${id}`); } catch (error) { target += `?error=${message(error)}`; } redirect(target); }
export async function generateProgramacionFlightsAction(form: FormData) { const id = value(form, "id"); let target = `/staff/operations/programacion/${id}`; try { const staff = await requireStaffPermission("SCHEDULE_STATUS_MANAGE", { entityType: "FlightSchedule", entityId: id, attemptedAction: "extend Programación horizon" }); const result = await generateFlightsForSchedule({ scheduleId: id, actor: staff }); target += `?generated=1&created=${result.created}&existing=${result.existing}`; revalidatePath(`/staff/operations/programacion/${id}`); } catch (error) { target += `?error=${message(error)}`; } redirect(target); }
