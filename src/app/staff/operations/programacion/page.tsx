...
const validDate = (value?: string) => { const parsed = value && /^\d{4}-\d{2}-\d{2}$/.test(value) ? new Date(`${value}T00:00:00Z`) : new Date(); return Number.isFinite(parsed.getTime()) ? parsed : new Date(); };
const queryString = (query: Record<string, string | undefined>, changes: Record<string, string | undefined>) => { const params = new URLSearchParams(); for (const [key, value] of Object.entries({ ...query, ...changes })) if (value) params.set(key, value); return `?${params}`; };
...
export default async function ProgramacionWorkspace({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const panelRequested = query.panel === "schedule" && (query.mode === "create" || query.mode === "edit");
  ...
  if (mayOpenPanel && query.mode === "create") {
    ...
    formValue = { code: query.code, routeId: reverseRoutes.length === 1 ? reverseRoutes[0].id : undefined, assignedAircraftId: query.aircraftId && query.aircraftId !== "unassigned" ? query.aircraftId : undefined, defaultFleetId: query.defaultFleetId || plannerData?.selectedAircraft?.nativeFleetId, daysOfWeek: query.daysOfWeek?.split(",").map(Number), departureTimeMinutesUtc: query.departureTimeMinutesUtc ? Number(query.departureTimeMinutesUtc) : undefined, effectiveFrom: query.effectiveFrom || iso(weekStart), effectiveUntil: query.effectiveUntil };
  }
