...
export async function syncAllPireps() {
  ...
  try {
    ...
    const message = `${result.importedCount} importados, ${result.updatedCount} actualizados y ${result.payrollGeneratedCount} nóminas generadas${result.errors.length ? `; ${result.errors.length} errores` : ""}.`;
