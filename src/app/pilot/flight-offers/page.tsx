...
export default async function PilotFlightOffersPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <PilotPortalShell>
    <div className="booking-page-header"><PageHeading eyebrow="HISPAFLY NATIVE" title="Available flights" copy="Book a published flight or create your own operation without any external booking system." /><Link className="button" href="/pilot/flight-offers/self-dispatch">Create my flight</Link></div>
    ...
    <section className="card"><div className="table-wrap"><table><thead><tr><th>Flight</th><th>Route</th><th>Operating date</th><th>Local schedule</th><th>Duration</th><th>Fleet / Aircraft</th><th>Window</th><th></th></tr></thead><tbody>{result.rows.map((flight) => <tr key={flight.id}><td><strong>{flight.flightNumber}</strong><br/>{flight.callsign}</td><td>{flight.departureIcao} → {flight.arrivalIcao}</td><td>{flight.operatingDate.toISOString().slice(0, 10)}</td><td>{flight.departureLocalTime} {flight.departureTimezone}<br/>{flight.arrivalLocalTime} {flight.arrivalTimezone}</td><td>{flight.scheduledDurationMinutes} min</td><td>{flight.fleet?.code ?? "Fleet pending"}<br/>{flight.assignedAircraft?.registration ?? "Aircraft pending assignment"}</td><td>{flight.bookingCloseAt ? `Closes ${dateTime(flight.bookingCloseAt)}` : "Open until departure"}</td><td><Link className="action-button approve" href={`/pilot/flight-offers/${flight.id}`}>Review & book</Link></td></tr>)}</tbody></table></div>{!result.rows.length && <div className="flight-availability-empty"><span>NO FLIGHTS AVAILABLE</span><h2>{scheduleCount === 0 ? "The flight programme has not been published yet" : futureFlightCount === 0 ? "No future flights have been generated" : "No flight matches the selected filters"}</h2><p>{scheduleCount === 0 ? "Routes are available, but Operations must publish a Schedule before pilots can book a concrete flight." : futureFlightCount === 0 ? "Operations has an active Schedule but still needs to generate its upcoming Flight instances." : "Clear one or more filters or select a wider date range."}</p>{(query.from || query.to || query.departureAirportId || query.arrivalAirportId || query.flightNumber || query.fleetId) && <Link className="button secondary" href="/pilot/flight-offers">Clear filters</Link>}</div>}</section>
  </PilotPortalShell>;
}
