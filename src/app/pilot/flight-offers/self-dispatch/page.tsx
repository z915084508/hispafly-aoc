...
export default async function NativeSelfDispatchPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  return <PilotPortalShell>
    ...
    <section className="card"><div className="workflow-summary"><div><span>Network</span><strong>{routeOptions.length} active routes</strong></div><div><span>Fleet readiness</span><strong>{aircraftOptions.filter((item) => item.airportId === position.airport!.id).length} aircraft at {position.airport.icao}</strong></div><div><span>Position control</span><strong>{position.airport.icao}</strong></div></div><p className="meta">Your departure airport is automatically locked to your current crew position. Only compatible aircraft at {position.airport.icao} are offered.</p></section>
