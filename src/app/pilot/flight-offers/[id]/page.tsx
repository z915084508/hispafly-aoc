...
export default async function FlightBookingDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return <PilotPortalShell><PageHeading eyebrow="PROGRAMACIÓN PUBLICADA" title={`${flight.flightNumber} · ${flight.departureIcao} → ${flight.arrivalIcao}`} copy="Revisa el vuelo programado antes de reservarlo."/>
    ...
    {availability.state === "MY_BOOKING" ? <Link className="button" href={`/pilot/bookings/${availability.bookingId}`}>ABRIR MI RESERVA</Link> : <form action={bookNativeFlightAction}><input type="hidden" name="flightId" value={flight.id}/><input type="hidden" name="idempotencyKey" value={randomUUID()}/>{!flight.assignedAircraftId && <label>Aeronave<select name="aircraftId" required><option value="">Selecciona una aeronave</option>{availableAircraft.map((item) => <option key={item.id} value={item.id}>{item.registration} · {item.currentAirport?.icao}</option>)}</select></label>}<p className="meta">La reserva no crea Dispatch ni OFP. Podrás cancelarla antes de preparar el Dispatch.</p><button className="button" disabled={!eligibility.allowed || availability.state !== "AVAILABLE"}>RESERVAR VUELO PROGRAMADO</button></form>}
