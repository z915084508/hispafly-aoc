...
export default async function PilotBookingsPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  return <PilotPortalShell>
    <div className="booking-page-header"><div><p className="eyebrow">DISPATCH PORTAL</p><h1>My Operations</h1><p className="page-copy">Manage every accepted mission and self-created flight through Dispatch, OFP, ACARS and PIREP.</p></div><Link className="button" href="/pilot/flight-offers/self-dispatch">Create my flight</Link></div>
    <div className="booking-overview"><div><span>Action required</span><strong>{current.filter((booking) => !booking.dispatch).length}</strong><small>Operations waiting for Dispatch</small></div><div><span>Active</span><strong>{current.length}</strong><small>Current AOC workflow</small></div><div><span>Completed</span><strong>{bookings.filter((booking) => completedStatuses.includes(booking.status)).length}</strong><small>Completed or flown</small></div><div><span>History</span><strong>{bookings.filter((booking) => booking.dataOrigin === "VAMSYS_LEGACY").length}</strong><small>Imported read-only records</small></div></div>
    <form className="booking-filter"><label>Status<select name="status" defaultValue={selectedStatus ?? ""}><option value="">All operations</option><optgroup label="Current"><option>CONFIRMED</option><option>DISPATCH_PENDING</option><option>DISPATCHED</option><option>IN_PROGRESS</option></optgroup><optgroup label="History"><option>COMPLETED</option><option>FLOWN</option><option>CANCELLED</option><option>EXPIRED</option><option>FAILED</option></optgroup></select></label><button className="button secondary">Apply filter</button>{selectedStatus && <Link className="action-button" href="/pilot/bookings">Clear</Link>}</form>
    <section className="booking-section"><div className="booking-section-title"><div><span>01</span><div><h2>Current operations</h2><p>Flights that can still move through Dispatch, OFP and ACARS.</p></div></div><b>{current.length}</b></div>{current.length ? <div className="pilot-booking-list">{current.map(bookingCard)}</div> : <div className="booking-empty"><strong>No active operation</strong><p>{selectedStatus ? "No operation matches this status." : "Accept a marketplace mission or create your own flight."}</p><div className="form-actions"><Link className="button" href="/pilot/flight-offers">Open Flight Marketplace</Link><Link className="button secondary" href="/pilot/flight-offers/self-dispatch">Create my flight</Link></div></div>}</section>
    <details className="booking-history" open={Boolean(selectedStatus && !activeStatuses.includes(selectedStatus))}><summary><span><b>02</b><strong>Operation history</strong><small>Completed, cancelled and imported vAMSYS records</small></span><em>{history.length}</em></summary>{history.length ? <div className="pilot-booking-list">{history.map(bookingCard)}</div> : <div className="empty-state">No historical operations match this filter.</div>}</details>
