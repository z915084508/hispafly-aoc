...
export async function signOFPAction(formData: FormData) {
  ...
  revalidatePath(`/pilot/ofp/${ofp.id}`); revalidatePath("/staff/ofp"); redirect(`/pilot/ofp/${ofp.id}?success=${encodeURIComponent("OFP signed and uploaded to AOC.")}`);
}
...
export async function finalDispatchOFPAction(formData: FormData) {
  ...
  try { const dispatch = await finalDispatchFlightOffer(dispatchId, pilot.id); resultId = dispatch.vamsysBookingId; revalidatePath("/pilot/flight-offers"); revalidatePath("/staff/flight-offers"); }
