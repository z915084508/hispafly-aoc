...
export default async function PilotsPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; hub?: string; success?: string; error?: string }> }) {
  ...
  const [pilots, recentPilots, hubCounts] = await Promise.all([
  ...
  return <>
    ...
    <div className="card">{pilots.length ? <DataTable headers={["Pilot", "HUB", "Rank", "Status", "PIREPs", "Identity", ""]} rows={rows} /> : <p className="meta">No pilots match this filter.</p>}</div>
  </>;
}
