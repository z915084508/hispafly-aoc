...
Accepted PIREPs are synchronized globally with the Operations endpoint `GET /pireps`, using `filter[status]=accepted`, cursor pagination with 50 results per page and `sort=-created_at`. Each list item is enriched through `GET /pireps/{pirep_id}` before payroll eligibility is evaluated. No Operations API mutation endpoint is used.

The company dashboard aggregates accepted monthly flights, passengers and passenger revenue. Revenue uses `PAX × 80 credits × distance factor` and is persisted in integer credit cents.
